...
import "../styles/CommissionPayouts.css"; // Import CSS
...
const CommissionPayouts = () => {
  ...
  const fetchPayouts = async () => {
    try {
      const response = await axios.get("/api/commission-payouts");
  ...
  const handleRequestPayout = async () => {
    try {
      await axios.post("/api/commission-payouts/request", {
        amount: payoutAmount,
  ...
  return (
    <div className="commission-payouts-container">
      <h2 className="text-primary mb-4">💰 Commission Payouts</h2>
      ...
      <style>
        {`
        /* Commission Payouts Styling */
.commission-payouts-container {
  padding: 20px;
  max-width: 90%;
  margin: auto;
}
